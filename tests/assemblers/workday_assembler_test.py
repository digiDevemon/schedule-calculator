...
def it_should_not_return_none():
    ...
    assert workday_assembler.get_workday_from_configuration(__SCHEDULE_CONFIG) is not None, "It should not return none"
...
def it_should_return_the_expected_type_object():
    ...
    assert type(workday_assembler.get_workday_from_configuration(
        __SCHEDULE_CONFIG)) is WorkDayCalculator, f"It should return an object with type {type(WorkDayCalculator)}"
