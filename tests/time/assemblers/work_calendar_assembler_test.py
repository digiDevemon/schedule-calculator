...
def it_should_not_return_none(time_formatter, clock):
    schedule_assembler = ScheduleAssembler(time_formatter, clock)
...
def it_should_return_the_expected_schedule_with_standard_time_value(time_formatter, clock):
    schedule = ScheduleAssembler(time_formatter, clock).get_schedule(__SCHEDULE_CONFIG)
...
def it_should_return_the_expected_schedule_with_continuous_delta_value(time_formatter, clock):
    schedule = ScheduleAssembler(time_formatter, clock).get_schedule(__SCHEDULE_CONFIG)
...
def it_should_return_the_expected_schedule_with_continuous_period_days_value(time_formatter, clock):
    schedule = ScheduleAssembler(time_formatter, clock).get_schedule(__SCHEDULE_CONFIG)
...
def it_should_return_a_schedule_without_continuous_period_days_value(time_formatter, clock):
    new_schedule = __SCHEDULE_CONFIG.copy()
    new_schedule.pop("continuous_period")
    schedule = ScheduleAssembler(time_formatter, clock).get_schedule(new_schedule)
